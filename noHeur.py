...
def select_move_minimax(board, color):
    ...
    while contin:
      ...
      origTime = time.time()
      if len(times) > 1:
        if sum(times) + (times[-1]/times[-2] * times[-1] + 2) > 5:
          contin = False
...
def run_ai():
    ...
    """
    print("CASSIO") # First line is the name of this AI  
...
def minimax_max_node(board, color, depth, alpha, beta):
    ...
    for i in range(len(moves)):
      if moves[i] in cornerList or moves[i] in sideList:
...
def run_ai():
    ...
    until the game is over. 
    """
    print("noHEUR") # First line is the name of this AI  
    color = int(input()) # Then we read the color: 1 for dark (goes first), 
                         # 2 for light. 
